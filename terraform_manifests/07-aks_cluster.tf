...
resource "azurerm_kubernetes_cluster" "aks_cluster" {
  name                = "${var.aks_environment}-aks-cluster"
  location            = var.location
  resource_group_name = "${var.aks_environment}-${var.resource_group_name}"

  dns_prefix          = "${var.aks_environment}-aks-cluster"
  kubernetes_version  = data.azurerm_kubernetes_service_versions.current.latest_version
  node_resource_group = "${var.aks_environment}-nrg"
...
azure_active_directory_role_based_access_control {
  ...
  admin_group_object_ids = [azuread_group.aks-administrators.object_id] # ADDED OCT2024
}
